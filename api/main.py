...
async def _dvic_reminder_loop():
    ...
    while True:
        try:
            ...
            db = SessionLocal()
            try:
                await asyncio.to_thread(dvic.run_dvic_ack_unlock_sweep, db)
            finally:
                db.close()
        except Exception as exc:
...
async def _ops_daily_digest_loop():
    ...
    which no-ops until the day's final Cortex data (delivered + RTS) has
    posted (same signal as ops_cadence.py's All In gate) and respects an
...
async def _survey_nudge_loop():
    """Every 60 s — delegates to surveys.run_survey_nudges(), which
    re-sends any active survey's link to anyone still incomplete, no
    more than once every 24h, indefinitely (no auto-escalation to a
    routing gate or a consequence -- see surveys.py's module docstring).
    Gated by SURVEY_NUDGE_ACTIVE (default false)."""
    while True:
        try:
            db = SessionLocal()
            try:
                await asyncio.to_thread(surveys.run_survey_nudges, db)
            except Exception as exc:
                logger.warning("Survey nudge loop error: %s", exc)
            finally:
                db.close()
        except Exception as exc:
            logger.warning("Survey nudge loop outer error: %s", exc)
        await asyncio.sleep(60)
...
async def startup():
    ...
    ensure_driver_employment_status_columns()
    ensure_dvic_ack_unlock_column()
    ensure_packages_record_column_widths()
    ensure_ops_ingest_job_attempts_column()
    ...
    ensure_coaching_notification_status_column()
    ...
    asyncio.create_task(_survey_nudge_loop())
...
app.include_router(team_room_monitor.router)
app.include_router(tenured_workforce.router)
