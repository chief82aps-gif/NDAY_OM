...
def build_column_map(
    ...
    """
    Build semantic field -> dataframe column index map with multi-layer detection:
    1. Header detection (look for column labels)
    2. Format detection (pattern matching on data)
    3. Fallback to positional defaults
    ...
    # Layer 2: Format/pattern detection (when no header found)
    # Only detect strongly-typed, unambiguous fields:
    # - route_code: very distinctive pattern (2-3 alpha + 2-5 digits)
    # - numeric fields: clearly numeric (duration, zones, packages)
    # Skip ambiguous fields like driver_name (company names look similar)
    # and dsp (various business entity formats)
    strongly_typed_fields = ["route_code", "route_duration", "num_zones", "num_packages"]
    column_map = detect_columns_by_format(df, column_map, strongly_typed_fields)
